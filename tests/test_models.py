...
def test_mlp_mixer_s16():
    model = mlp_mixer.models.mlp_mixer_s16(**PARAMS)
...
def test_mlp_mixer_s32():
    model = mlp_mixer.models.mlp_mixer_s32(**PARAMS)
    model = model.to(DEVICE)
    y_hat = model(x)
    assert y_hat.shape == (1, PARAMS["num_classes"])
    assert y_hat.dtype == torch.float32
...
def test_mlp_mixer_b16():
    model = mlp_mixer.models.mlp_mixer_b16(**PARAMS)
    model = model.to(DEVICE)
    y_hat = model(x)
    assert y_hat.shape == (1, PARAMS["num_classes"])
    assert y_hat.dtype == torch.float32
...
def test_mlp_mixer_b32():
    model = mlp_mixer.models.mlp_mixer_b32(**PARAMS)
    model = model.to(DEVICE)
    y_hat = model(x)
    assert y_hat.shape == (1, PARAMS["num_classes"])
    assert y_hat.dtype == torch.float32
...
def test_mlp_mixer_l16():
    model = mlp_mixer.models.mlp_mixer_l16(**PARAMS)
    model = model.to(DEVICE)
    y_hat = model(x)
    assert y_hat.shape == (1, PARAMS["num_classes"])
    assert y_hat.dtype == torch.float32
...
def test_mlp_mixer_l32():
    model = mlp_mixer.models.mlp_mixer_l32(**PARAMS)
    model = model.to(DEVICE)
    y_hat = model(x)
    assert y_hat.shape == (1, PARAMS["num_classes"])
    assert y_hat.dtype == torch.float32
...
def test_mlp_mixer_h14():
    model = mlp_mixer.models.mlp_mixer_h14(**PARAMS)
    model = model.to(DEVICE)
    y_hat = model(x)
    assert y_hat.shape == (1, PARAMS["num_classes"])
    assert y_hat.dtype == torch.float32
